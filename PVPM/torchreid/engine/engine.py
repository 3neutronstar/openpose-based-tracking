...
class Engine(object):
    ...
    def run(self, save_dir='log', max_epoch=0, start_epoch=0, fixbase_epoch=0, open_layers=None,
            ...
            use_metric_cuhk03=False, ranks=[1, 5, 10, 20], rerank=False, load_pose=False, part_score=False):
    ...
    def test(self, epoch, testloader, dist_metric='euclidean', normalize_feature=False,
             ...
             ranks=[1, 5, 10, 20], rerank=False, load_pose=False, part_score=False):
    ...
    def _evaluate(self, epoch, dataset_name='', queryloader=None, galleryloader=None,
                  ...
                  rerank=False, load_pose=False, part_score=False):
        ...
        if part_score:
            distmat=metrics.compute_weight_distance_matrix(qf,gf,q_score,g_score,dist_metric)
        else:
    ...
    def _compute_loss(self, criterion, outputs, targets, part_weights=None):
        if isinstance(outputs, (tuple, list)):
            loss = DeepSupervision(criterion, outputs, targets, part_weights=part_weights)
        else:
